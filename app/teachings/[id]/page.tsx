...
export default async function TeachingPage({ params }: Props) {
  ...
  const { level, billingInterval, adminLevel } = await getMembership()
  ...
  const allowPrint = canAccessOrAdmin(
    level,
    billingInterval,
    adminLevel,
    'print_teaching',
    ...
    <main id="reading-root" className="min-h-screen bg-[#F7F4EF]">
      ...
      <div className="max-w-3xl mx-auto px-6 pt-8 pb-8 print-sheet">
        ...
        {(teaching.closing_phrase || teaching.end_time) && (
          <div className="mt-12 print-closing">
            {teaching.closing_phrase && (
              <div className="text-[#2C2522] phrase text-right">
                {teaching.closing_phrase}
              </div>
            )}
            {teaching.end_time && (
              <div className="text-[#6B5E54] mt-1 time text-right">
                {teaching.end_time}
              </div>
            )}
          </div>
